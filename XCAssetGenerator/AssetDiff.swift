...
struct AssetDiff {
    ...
    static func diffWithOperation(source: [Asset], catalog: AssetCatalog)(operation: DiffOperation) -> Int {
        let s = source.map { Comparison(path: $0.fullPath, key: $0.fullPath.remove([$0.ancestor.removeTrailingSlash()])) }
        
        let c = PathQuery.availableImages(from: catalog.path).map { path in
            return Comparison(path: path, key: path.remove([catalog.path]).removeAssetSetsComponent())
        }
    ...
    private static func common(source: [Comparison], target: [Comparison]) -> Int {
        var total = 0
        for c in target {
            total += source.filter { $0.key == c.key && FileSystem.equal($0.path, c.path) }.count
        }
        return total
    }
    ...
    private static func new(source: [Comparison], target: [Comparison]) -> Int {
        ...
        for s in source {
            new += target.filter { $0.key == s.key && FileSystem.equal($0.path, s.path) }.count == 0 ? 1 : 0
        }
        return new
    }
